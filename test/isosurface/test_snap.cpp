...
TEST(snap, points_inside_bbox_become_vertices) {
  ...
  auto mesh = snap_mesh(planar_grid(3), points, kCenterBbox, kMaxDistance);
...
TEST(snap, manifold_and_disk_topology) {
  auto mesh = snap_mesh(planar_grid(3), center_points(), kCenterBbox, kMaxDistance);
...
TEST(snap, leaves_boundary_untouched) {
  ...
  auto mesh = snap_mesh(grid, center_points(), kCenterBbox, kMaxDistance);
...
TEST(snap, empty_points_is_noop) {
  auto grid = planar_grid(3);
  Points3 points(0, 3);
  auto mesh = snap_mesh(grid, points, kCenterBbox, kMaxDistance);
...
TEST(snap, rejects_points_beyond_max_distance) {
  ...
  auto rejected = snap_mesh(planar_grid(3), points, kCenterBbox, 0.2);
  ASSERT_FALSE(find_vertex(rejected, Point3(points.row(0)), 1e-12).has_value());

  auto accepted = snap_mesh(planar_grid(3), points, kCenterBbox, 0.4);
