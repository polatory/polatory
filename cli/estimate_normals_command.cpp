...
class EstimateNormalsCommand : public Command {
  static inline const std::string kDescription = "Estimate normals of a point cloud";
  static inline const std::string kName = "estimate-normals";
  ...
  void run(const std::vector<std::string>& args, const GlobalOptions& global_opts) const override {
    ...
    opts_desc.add_options()  //
        ...
        ("closed", po::value(&opts.k_closed)->value_name("K"),
         "Orient normals of closed surface(s) using k-NN search with the specified number of "
         "points\n"
...
CommandPtr make_estimate_normals_command() { return std::make_unique<EstimateNormalsCommand>(); }
