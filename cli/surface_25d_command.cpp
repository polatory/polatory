...
class Surface25DCommand : public Command {
  static inline const std::string kDescription =
      "Generate a 2.5D surface mesh from a 2D interpolant";
  static inline const std::string kName = "surface-25d";
 ...
 private:
  ...
  static void run_impl(const Options& opts) {
    ...
    auto mesh = seed_points.rows() > 0
                    ? isosurf.generate_from_seed_points(seed_points, field_fn, 0.0)
...
CommandPtr make_surface_25d_command() { return std::make_unique<Surface25DCommand>(); }
