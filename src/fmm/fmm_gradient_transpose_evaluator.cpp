...
#include "gradient_transpose_kernel.hpp"

namespace polatory::fmm {

template <int Order, int Dim>
class fmm_gradient_transpose_evaluator<Order, Dim>::impl {
  ...
  using Kernel = gradient_transpose_kernel<Dim>;
...
template <int Order, int Dim>
fmm_gradient_transpose_evaluator<Order, Dim>::fmm_gradient_transpose_evaluator(
    const model& model, int tree_height, const geometry::bbox3d& bbox)
    : pimpl_(std::make_unique<impl>(model, tree_height, bbox)) {}

template <int Order, int Dim>
fmm_gradient_transpose_evaluator<Order, Dim>::~fmm_gradient_transpose_evaluator() = default;

template <int Order, int Dim>
common::valuesd fmm_gradient_transpose_evaluator<Order, Dim>::evaluate() const {
  return pimpl_->evaluate();
}

template <int Order, int Dim>
void fmm_gradient_transpose_evaluator<Order, Dim>::set_field_points(
    const geometry::points3d& points) {
  pimpl_->set_field_points(points);
}

template <int Order, int Dim>
void fmm_gradient_transpose_evaluator<Order, Dim>::set_source_points(
    const geometry::points3d& points) {
  pimpl_->set_source_points(points);
}

template <int Order, int Dim>
void fmm_gradient_transpose_evaluator<Order, Dim>::set_source_points_and_weights(
...
template <int Order, int Dim>
void fmm_gradient_transpose_evaluator<Order, Dim>::set_weights(
    const Eigen::Ref<const common::valuesd>& weights) {
  pimpl_->set_weights(weights);
}

template class fmm_gradient_transpose_evaluator<6, 1>;
template class fmm_gradient_transpose_evaluator<10, 1>;
template class fmm_gradient_transpose_evaluator<6, 2>;
template class fmm_gradient_transpose_evaluator<10, 2>;
template class fmm_gradient_transpose_evaluator<6, 3>;
template class fmm_gradient_transpose_evaluator<10, 3>;
