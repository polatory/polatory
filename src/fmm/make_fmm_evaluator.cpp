...
#define CASE(RBF_NAME)                                                     \
  if (auto* derived = dynamic_cast<RBF_NAME<Dim>*>(base)) {                \
    return std::make_unique<fmm_evaluator<RBF_NAME<Dim>>>(*derived, bbox); \
  }
...
#define CASE(RBF_NAME)                                                              \
  if (auto* derived = dynamic_cast<RBF_NAME<Dim>*>(base)) {                         \
    return std::make_unique<fmm_gradient_evaluator<RBF_NAME<Dim>>>(*derived, bbox); \
  }
...
#define CASE(RBF_NAME)                                                             \
  if (auto* derived = dynamic_cast<RBF_NAME<Dim>*>(base)) {                        \
    return std::make_unique<fmm_hessian_evaluator<RBF_NAME<Dim>>>(*derived, bbox); \
  }
...
#define CASE(RBF_NAME)                                                               \
  if (auto* derived = dynamic_cast<RBF_NAME<Dim>*>(base)) {                          \
    return std::make_unique<fmm_symmetric_evaluator<RBF_NAME<Dim>>>(*derived, bbox); \
  }
