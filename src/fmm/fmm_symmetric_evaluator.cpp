...
class fmm_generic_symmetric_evaluator<Model, Order, Kernel>::impl {
...
template <class Model, int Order, class Kernel>
fmm_generic_symmetric_evaluator<Model, Order, Kernel>::fmm_generic_symmetric_evaluator(
    const Model& model, const geometry::bbox3d& bbox)
    : impl_(std::make_unique<impl>(model, bbox)) {}

template <class Model, int Order, class Kernel>
fmm_generic_symmetric_evaluator<Model, Order, Kernel>::~fmm_generic_symmetric_evaluator() = default;

template <class Model, int Order, class Kernel>
common::valuesd fmm_generic_symmetric_evaluator<Model, Order, Kernel>::evaluate() const {
  return impl_->evaluate();
}

template <class Model, int Order, class Kernel>
void fmm_generic_symmetric_evaluator<Model, Order, Kernel>::set_points(
    const geometry::points3d& points) {
  impl_->set_points(points);
}

template <class Model, int Order, class Kernel>
void fmm_generic_symmetric_evaluator<Model, Order, Kernel>::set_weights(
    const Eigen::Ref<const common::valuesd>& weights) {
  impl_->set_weights(weights);
}

#define IMPLEMENT3(MODEL, DIM, ORDER)                                                    \
  template class fmm_generic_symmetric_evaluator<MODEL, ORDER,                           \
                                                 kernel<typename MODEL::rbf_type, DIM>>; \
  template class fmm_generic_symmetric_evaluator<MODEL, ORDER,                           \
