...
public:
  rbf_evaluator(const rbf::rbf& rbf, const geometry::points3d& source_points)
    : n_poly_basis_(rbf.poly_basis_size()) {
    auto bbox = geometry::bbox3d::from_points(source_points);

    a_ = std::make_unique<fmm::fmm_evaluator<Order>>(rbf, fmm::fmm_tree_height(source_points.rows()), bbox);
  ...
  rbf_evaluator(const rbf::rbf& rbf, const geometry::points3d& source_points, const geometry::bbox3d& bbox)
    : n_poly_basis_(rbf.poly_basis_size()) {
    a_ = std::make_unique<fmm::fmm_evaluator<Order>>(rbf, fmm::fmm_tree_height(source_points.rows()), bbox);
  ...
  rbf_evaluator(const rbf::rbf& rbf, int tree_height, const geometry::bbox3d& bbox)
    : n_poly_basis_(rbf.poly_basis_size())
    , n_src_points_(0) {
