...
public:
  ...
  explicit cov_spheroidal5(const std::vector<double>& params) {
  ...
  static double evaluate_untransformed(double r, const double *params) {
    ...
    return r < 0.25801274118035729 * range
           ...
           : psill * 0.85759801680320064 * std::pow(1.0 + 2.5036086535164204 * std::pow(r / range, 2.0), -2.5);
  }
  ...
  void evaluate_gradient_untransformed(
    ...
    auto c = r < 0.25801274118035729 * range
             ...
             : -psill * 10.735449080535068 * std::pow(1.0 + 2.5036086535164204 * std::pow(r / range, 2.0), -3.5) / (range * range);
    *gradx = c * x;
    *grady = c * y;
    *gradz = c * z;
  }
